...
(function () {
  ...
  /* ---------- Popup spacing guard ----------
     Keeps the cookie banner and exit-intent modal from ever
  ...
  function initCookieConsent() {
    ...
    var allowBtn = document.getElementById('cookieAllow');
    var closeBtn = document.getElementById('cookieClose');
    if (!overlay || !allowBtn || !closeBtn) return;

    var STORAGE_KEY = 'derila_cookie_consent';
    var alreadyAnswered = false;
    try { alreadyAnswered = !!window.localStorage.getItem(STORAGE_KEY); } catch (err) { alreadyAnswered = false; }
    if (alreadyAnswered) return;

    function reveal() {
      if (!canShowPopupNow(reveal)) return;
      overlay.hidden = false;
    }
    ...
    function answer(value) {
      overlay.hidden = true;
      try { window.localStorage.setItem(STORAGE_KEY, value); } catch (err) { /* no-op */ }
    }

    allowBtn.addEventListener('click', function () { answer('allowed'); });
    closeBtn.addEventListener('click', function () { answer('dismissed'); });
  }
